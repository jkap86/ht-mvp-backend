...
-- Non-concurrent version for easier migration (can be rebuilt with CONCURRENTLY later in production)

-- Full-text search index for league chat messages
-- Uses GIN index for fast full-text search on message content
CREATE INDEX IF NOT EXISTS idx_league_chat_messages_fts
...
CREATE INDEX IF NOT EXISTS idx_league_chat_messages_league_timestamp
...
CREATE INDEX IF NOT EXISTS idx_league_chat_messages_type
