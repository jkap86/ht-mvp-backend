...
const createMockPlayoffRepo = (): jest.Mocked<PlayoffRepository> =>
  ({
    ...
    getFinalizedSeriesEndingInWeek: jest.fn().mockResolvedValue([]),
    ...
    areAllSeriesCompleteForRound: jest.fn().mockResolvedValue(true),
...
describe('PlayoffService', () => {
  ...
  describe('6-team consolation bracket with byes', () => {
    ...
    it('should advance 6-team consolation with bye teams in round 2', async () => {
      ...
      // Mock series aggregation (for multi-week series support)
      // Since WINNERS has no matchups, only CONSOLATION's getFinalizedSeriesEndingInWeek is called
      mockPlayoffRepo.getFinalizedSeriesEndingInWeek
        .mockResolvedValueOnce(round1Matchups.map(m => ({
          seriesId: `series-${m.id}`,
          roster1Id: m.roster1_id,
          roster2Id: m.roster2_id,
          roster1TotalPoints: m.roster1_points,
          roster2TotalPoints: m.roster2_points,
          roster1Seed: m.playoff_seed1,
          roster2Seed: m.playoff_seed2,
          gamesCompleted: 1,
          seriesLength: 1,
          isComplete: true,
        }))); // CONSOLATION - completed series
      mockPlayoffRepo.areAllSeriesCompleteForRound = jest.fn().mockResolvedValue(true);
      ...
      // Verify round 2 matchups were created with bye teams (uses createPlayoffMatchupWithSeries now)
      expect(mockPlayoffRepo.createPlayoffMatchupWithSeries).toHaveBeenCalledTimes(2);

      // Semifinal 1: Seed 1 (bye) vs winner of 4v5 (seed 4)
      // createPlayoffMatchupWithSeries(leagueId, season, week, roster1, roster2, round, bracketPos, seed2, seed1, bracketType, seriesId, game, totalGames, client)
      const semifinal1 = mockPlayoffRepo.createPlayoffMatchupWithSeries.mock.calls[0];
      expect(semifinal1[3]).toBe(101); // seed 1 roster
      expect(semifinal1[4]).toBe(104); // winner of 4v5
      expect(semifinal1[5]).toBe(2); // round 2
      expect(semifinal1[6]).toBe(1); // bracket position 1

      // Semifinal 2: Seed 2 (bye) vs winner of 3v6 (seed 3)
      const semifinal2 = mockPlayoffRepo.createPlayoffMatchupWithSeries.mock.calls[1];
      expect(semifinal2[3]).toBe(102); // seed 2 roster
      expect(semifinal2[4]).toBe(103); // winner of 3v6
      expect(semifinal2[5]).toBe(2); // round 2
      expect(semifinal2[6]).toBe(2); // bracket position 2
    });
  });
  ...
  describe('bracket completion semantics', () => {
    it('should not complete bracket when only champion is set (3rd place pending)', async () => {
      ...
      // Mock series aggregation
      mockPlayoffRepo.getFinalizedSeriesEndingInWeek
        .mockResolvedValueOnce(championshipMatchups.map(m => ({
          seriesId: `series-${m.id}`,
          roster1Id: m.roster1_id,
          roster2Id: m.roster2_id,
          roster1TotalPoints: m.roster1_points,
          ...
          seriesLength: 1,
          isComplete: true,
        }))) // WINNERS - championship series completed
        .mockResolvedValueOnce([]) // CONSOLATION
        .mockResolvedValueOnce([]); // THIRD_PLACE
      mockPlayoffRepo.areAllSeriesCompleteForRound = jest.fn().mockResolvedValue(true);
    ...
    it('should complete bracket when all required winners are set', async () => {
      ...
      // Mock series aggregation - only CONSOLATION is processed since WINNERS is empty
      mockPlayoffRepo.getFinalizedSeriesEndingInWeek
        .mockResolvedValueOnce(consolationFinalMatchups.map(m => ({
          seriesId: `series-${m.id}`,
          roster1Id: m.roster1_id,
          roster2Id: m.roster2_id,
          roster1TotalPoints: m.roster1_points,
          ...
          seriesLength: 1,
          isComplete: true,
        }))); // CONSOLATION - final series completed
      mockPlayoffRepo.areAllSeriesCompleteForRound = jest.fn().mockResolvedValue(true);
  ...
  describe('multi-week series advancement', () => {
    it('should advance winners based on aggregate scoring', async () => {
      ...
      mockLeagueRepo.findById.mockResolvedValue({ id: 1, season: '2024' } as any);
      mockPlayoffRepo.findByLeagueSeason.mockResolvedValue(mockBracket);
      // Need to mock getFinalizedMatchupsForWeekByType for the outer call
      mockPlayoffRepo.getFinalizedMatchupsForWeekByType
        .mockResolvedValueOnce([{ id: 1, playoff_round: 1 }]) // WINNERS - has matchups
        .mockResolvedValueOnce([]) // CONSOLATION - none
        .mockResolvedValueOnce([]); // THIRD_PLACE - none
    ...
    it('should use lower seed as tiebreaker for aggregate ties', async () => {
      ...
      // For 4-team: we need 2 series from round 1 (1v4 and 2v3)
      const seriesAggregation: SeriesAggregation[] = [
        {
          seriesId: 'series-1',
          roster1Id: 1,
          roster2Id: 4,
          roster1TotalPoints: 200, // Tied - seed 1 wins tiebreaker
          roster2TotalPoints: 200,
          roster1Seed: 1,
          roster2Seed: 4,
          gamesCompleted: 2,
          seriesLength: 2,
        ...
        },
        {
          ...
          roster1Id: 2,
          roster2Id: 3,
          roster1TotalPoints: 180, // Seed 2 wins with higher points
          roster2TotalPoints: 160,
          roster1Seed: 2,
          roster2Seed: 3,
          gamesCompleted: 2,
          seriesLength: 2,
      ...
      // Seed 1 should advance (tiebreaker) - verify createPlayoffMatchupWithSeries was called
      expect(mockPlayoffRepo.createPlayoffMatchupWithSeries).toHaveBeenCalled();
      // The winner (roster 1, seed 1) should be in the next round matchup
      // createPlayoffMatchupWithSeries(leagueId, season, week, roster1, roster2, round, ...)
      const createCalls = mockPlayoffRepo.createPlayoffMatchupWithSeries.mock.calls;
      // Roster 1 should appear as one of the participants in the next round
      const allRosterIds = createCalls.flatMap(call => [call[3], call[4]]);
      expect(allRosterIds).toContain(1); // Roster 1 should advance due to lower seed tiebreaker
    });
  });
});
