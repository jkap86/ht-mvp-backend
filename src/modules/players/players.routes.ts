...
// GET /api/players
router.get('/', apiReadLimiter, asyncHandler(playerController.getAllPlayers));

// GET /api/players/search?q=<query>&position=<pos>&team=<team>
router.get('/search', apiReadLimiter, asyncHandler(playerController.searchPlayers));
...
router.post('/sync-college', playerSyncLimiter, asyncHandler(playerController.syncCollegePlayers));

// ========== NEWS ROUTES (Stream A) ==========
// GET /api/players/news/latest - Latest news across all players
router.get('/news/latest', apiReadLimiter, asyncHandler(playerController.getLatestNews));

// GET /api/players/news/breaking - Breaking news (critical/high impact)
router.get('/news/breaking', apiReadLimiter, asyncHandler(playerController.getBreakingNews));
...
router.get('/:id/news', apiReadLimiter, asyncHandler(playerController.getPlayerNews));
...
router.get('/:id/gamelogs', apiReadLimiter, asyncHandler(playerController.getPlayerGameLogs));

// GET /api/players/:id/projections - Weekly projection
router.get('/:id/projections', apiReadLimiter, asyncHandler(playerController.getPlayerProjection));

// GET /api/players/:id/trends - Performance trends
router.get('/:id/trends', apiReadLimiter, asyncHandler(playerController.getPlayerTrends));
