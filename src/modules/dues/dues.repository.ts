...
export class DuesRepository {
  ...
  async getDuesConfig(leagueId: number, leagueSeasonId?: number): Promise<LeagueDues | null> {
    const result = leagueSeasonId
      ? await this.db.query(
          'SELECT * FROM league_dues WHERE league_season_id = $1',
          [leagueSeasonId]
        )
      : await this.db.query(
  ...
  async upsertDuesConfig(params: UpsertDuesConfigParams): Promise<LeagueDues> {
    const result = params.leagueSeasonId
      ? await this.db.query(
          `INSERT INTO league_dues (league_id, league_season_id, buy_in_amount, payout_structure, currency, notes)
           VALUES ($1, $2, $3, $4, $5, $6)
   ...
   */
  async deleteDuesConfig(leagueId: number, leagueSeasonId?: number): Promise<boolean> {
    return runInTransaction(this.db, async (client) => {
      const result = leagueSeasonId
        ? await client.query(
            'DELETE FROM league_dues WHERE league_season_id = $1',
            [leagueSeasonId]
          )
        : await client.query(
      ...
      if (leagueSeasonId) {
        await client.query('DELETE FROM dues_payments WHERE league_season_id = $1', [leagueSeasonId]);
      } else {
   ...
   */
  async getPaymentStatuses(leagueId: number, leagueSeasonId?: number): Promise<DuesPaymentWithRoster[]> {
    const result = leagueSeasonId
      ? await this.db.query(
          `SELECT
            dp.*,
           ...
           LEFT JOIN dues_payments dp ON dp.roster_id = r.id AND dp.league_season_id = r.league_season_id
           WHERE r.league_season_id = $1 AND r.is_benched = false
           ORDER BY team_name`,
          [leagueSeasonId]
        )
      : await this.db.query(
    ...
    return result.rows.map((row) => ({
      id: row.id || 0,
      leagueId: leagueId,
      leagueSeasonId: leagueSeasonId || row.league_season_id || 0,
   ...
   */
  async getPaymentStatus(leagueId: number, rosterId: number, leagueSeasonId?: number): Promise<DuesPayment | null> {
    const result = leagueSeasonId
      ? await this.db.query(
          'SELECT * FROM dues_payments WHERE league_season_id = $1 AND roster_id = $2',
          [leagueSeasonId, rosterId]
        )
      : await this.db.query(
  ...
  async markPaymentStatus(params: MarkPaymentParams): Promise<DuesPayment> {
    ...
    const result = params.leagueSeasonId
      ? await this.db.query(
          `INSERT INTO dues_payments (league_id, league_season_id, roster_id, is_paid, paid_at, marked_by_user_id, notes)
           VALUES ($1, $2, $3, $4, ${params.isPaid ? 'CURRENT_TIMESTAMP' : 'NULL'}, $5, $6)
   ...
   */
  async getPaymentSummary(leagueId: number, leagueSeasonId?: number): Promise<{ paidCount: number; totalCount: number }> {
    const result = leagueSeasonId
      ? await this.db.query(
          `SELECT
